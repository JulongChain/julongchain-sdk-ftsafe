...
import org.bcia.javachain.common.ledger.blockledger.BlockConstant;
...
public class LedgerHeight implements Comparable<LedgerHeight> {
    ...
    public LedgerHeight(byte[] b){
        this.blockNum = Util.bytesToLong(b, 0, BlockConstant.PEEK_BYTES_LEN);
        this.txNum = Util.bytesToLong(b, 8, BlockConstant.PEEK_BYTES_LEN);
    }
    ...
    public byte[] toBytes(){
        byte[] blockNumBytes = Util.longToBytes(blockNum, BlockConstant.PEEK_BYTES_LEN);
        byte[] txNumBytes = Util.longToBytes(txNum, BlockConstant.PEEK_BYTES_LEN);
