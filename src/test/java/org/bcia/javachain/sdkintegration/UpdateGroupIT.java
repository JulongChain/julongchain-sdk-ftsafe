...
//        assertEquals(256, Config.getConfig().getSecurityLevel());
    ...
    public void setup() {
        ...
        try {
            ...
            final byte[] channelConfigurationBytes = fooGroup.getGroupConfigurationBytes();
            ...
            HttpEntity multipartEntity = MultipartEntityBuilder.create()
                    ...
                    .addBinaryBody("channel", fooGroup.getName().getBytes()).build();

            httppost.setEntity(multipartEntity);

            response = httpclient.execute(httppost);
            statuscode = response.getStatusLine().getStatusCode();
            out("Got %s status for updated config bytes needed for updateGroupConfiguration ", statuscode);
            assertEquals(200, statuscode);

            byte[] updateBytes = EntityUtils.toByteArray(response.getEntity());

            UpdateGroupConfiguration updateGroupConfiguration = new UpdateGroupConfiguration(updateBytes);
            ...
            final SampleUser ordererAdmin = sampleStore.getMember(sampleOrgName + "OrderAdmin", sampleOrgName, "ConsenterMSP",
            ...
            fooGroup.updateGroupConfiguration(updateGroupConfiguration, client.getUpdateGroupConfigurationSignature(updateGroupConfiguration, ordererAdmin));

            //Let's add some additional verification...

            client.setUserContext(sampleOrg.getNodeAdmin());

            final byte[] modGroupBytes = fooGroup.getGroupConfigurationBytes();
            ...
            httppost.setEntity(new ByteArrayEntity(modGroupBytes));
    ...
    private Group reconstructGroup(String name, HFClient client, SampleOrg sampleOrg) throws Exception {
        ...
        client.setUserContext(sampleOrg.getNodeAdmin());
        Group newGroup = client.newGroup(name);

        for (String orderName : sampleOrg.getConsenterNames()) {
            newGroup.addConsenter(client.newConsenter(orderName, sampleOrg.getConsenterLocation(orderName),
                    testConfig.getConsenterProperties(orderName)));
        }

        for (String peerName : sampleOrg.getNodeNames()) {
            String peerLocation = sampleOrg.getNodeLocation(peerName);
            Node peer = client.newNode(peerName, peerLocation, testConfig.getNodeProperties(peerName));

            //Query the actual peer for which channels it belongs to and check it belongs to this channel
            Set<String> channels = client.queryGroups(peer);
            if (!channels.contains(name)) {
                throw new AssertionError(format("Node %s does not appear to belong to channel %s", peerName, name));
            }

            newGroup.addNode(peer, createNodeOptions().setNodeRoles(EnumSet.of(Node.NodeRole.CHAINCODE_QUERY,
                    Node.NodeRole.ENDORSING_PEER, Node.NodeRole.LEDGER_QUERY)));
        }
