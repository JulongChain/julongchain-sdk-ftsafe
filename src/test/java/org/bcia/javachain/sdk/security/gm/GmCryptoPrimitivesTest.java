...
import static org.bcia.javachain.sdk.testutils.TestUtils.getField;
import static org.bcia.javachain.sdk.testutils.TestUtils.setConfigProperty;
...
import org.bcia.javachain.sdk.GmTestConfig;
...
set up...
test keyGen...
07-16 23:10:14,148 [CspManager]-[WARN] Before using CSP, please call initCspFactories(). Falling back to bootCsp.
07-16 23:10:14,381 [CspManager]-[INFO] Initialize CSP gm
07-16 23:10:14,386 [CspManager]-[INFO] Initialize CSP sdtgm
07-16 23:10:14,465 [SM2]-[INFO] publicKey: 040551695f19348fea0e8af9d9c1a1cbfb172231da85beacad99a5ee5e4551a48964cd6b9d6c279b71b924f84b350efecf3b38d5e1818fed94d50d99b78b79c3e4
07-16 23:10:14,466 [SM2]-[INFO] privateKey: 18d7d61d9be36025dab616716df472a4e9f00ee8ce29630e444db898f9f96025
generate the SM2 Privatekey:18d7d61d9be36025dab616716df472a4e9f00ee8ce29630e444db898f9f96025
07-16 23:10:14,474 [SM2PublicKey]-[INFO] return the publicKey
generate the SM2 PublicKey:040551695f19348fea0e8af9d9c1a1cbfb172231da85beacad99a5ee5e4551a48964cd6b9d6c279b71b924f84b350efecf3b38d5e1818fed94d50d99b78b79c3e4
SM4 Key:f869392b3d9934063b525d280d9a5199
finalize...
 * */

public class GmCryptoPrimitivesTest {
...
//    public static final String PEM_PRIVATEKEY_BASE64 = "MHcCAQEEIFjkTJhmqB7QqN6gFP5GQpueif+t8sPS8V2ar+VqkZYXoAoGCCqBHM9VAYItoUQDQgAE5BN1Sx0+0GuhSg7hIzGWGC83GejF7J9cxZ1ODSJJcOQ/vV4VDnT6cBpY+mfYNlrMQ0deZobRBOpFBdlLkhw7kw==";
//    public static final String PEM_CERT_BASE64 = "";
//    public static final String INVALID_PEM_CERT_BASE64 = "";
//    public static final String PLAIN_TEXT_BASE64 = "";
//    public static final String SIGNATURE_BASE64 = "";
...
//    private static final String SIGNING_ALGORITHM = "SM3withSM2";

    // File create_key_cert_for_testing.md has info on the other keys and
    // certificates used in this test suite

    private static byte[] plainText, sig, bytePemCert, invalidPemCert;

    private static KeyFactory kf;

    private static CertificateFactory cf;

    private static GmCryptoPrimitives crypto;
    ...
    public static void setUpBeforeClass() throws Exception {
        config = GmTestConfig.getConfig();

        plainText = DatatypeConverter.parseHexBinary(config.getValue("plaintext.hex"));
        //sig = DatatypeConverter.parseHexBinary(config.getValue("signature.hex"));
        bytePemCert = config.getResourceAsBytes(config.getValue("certificate.gm.issuer.file.path"));
        invalidPemCert = DatatypeConverter.parseBase64Binary(config.getValue("certificate.gm.issuer.text.invalid"));

        kf = new KeyFactory();
        cf = new CertificateFactory();
        crypto = new GmCryptoPrimitives();
        crypto.init();
    ...
    public void setUp() throws Exception {
        ...
        byte[] bytes = config.getResourceAsBytes(config.getValue("certificate.gm.root.file.path"));
		testCACert = crypto.getX509Certificate(bytes);
        crypto.addCACertificateToTrustStore(testCACert, "ca");
        
		Certificate cert = crypto.getX509Certificate(bytePemCert);

		PEMParser pem = new PEMParser(new StringReader(config.getValue("certificate.gm.root.privatekey.pem")));
...
//---------wangzhe test----------
//        System.err.println("_________key_______"+ key);
//        PrivateKey newKey = (PrivateKey) crypto.getTrustStore().getKey("key", "123456".toCharArray());
//        System.err.println("________newkey________"+ newKey);
//        System.err.println("_____cert______"+ testCACert);
//        
//        byte[] signature = crypto.sign(key, "123456".getBytes());
//        boolean verifyResult = crypto.verify(testCACert, "SM3withSM2", signature, "123456".getBytes());
//        System.err.println("_____verifyResult______"+ verifyResult);
      //---------wangzhe test----------
    ...
    public void testInitInvalidCertFormat() throws Exception {
        ...
        try {
            ...
            oldVal = setConfigProperty(GmTestConfig.CERTIFICATE_FORMAT, "abc123");

            GmCryptoPrimitives crypto = new GmCryptoPrimitives();
            crypto.init();

        } finally {

            // Reset the property for subsequent tests
            setConfigProperty(GmTestConfig.CERTIFICATE_FORMAT, oldVal);
        }
    }
    ...
    public void testDefaultCrypto() throws Exception {
        ...
        CryptoSuite cryptoSuite = GmCryptoSuiteFactory.getDefault().getCryptoSuite();
        assertEquals("sm2p256v1", getField(cryptoSuite, "curveName"));
        assertEquals(256, getField(cryptoSuite, "securityLevel"));
        assertEquals("SM3", getField(cryptoSuite, "hashAlgorithm"));
        ...
        assertEquals(cryptoSuite, GmCryptoSuiteFactory.getDefault().getCryptoSuite());
    }
    ...
    public void testGetSetProperties() throws Exception {
        ...
        try {
            final String expectHash = "SM3"; // use something different than default!
            propsIn.setProperty(GmTestConfig.SECURITY_LEVEL, "384");
            propsIn.setProperty(GmTestConfig.HASH_ALGORITHM, expectHash);
            //    testCrypto.setProperties(propsIn);
            //   testCrypto.init();
            CryptoSuite testCrypto = GmCryptoSuiteFactory.getDefault().getCryptoSuite(propsIn);
            ...
            Properties cryptoProps = ((GmCryptoPrimitives) testCrypto).getProperties();
            assertEquals(cryptoProps.getProperty(GmTestConfig.SECURITY_LEVEL), "384");
            cryptoProps = testCrypto.getProperties();
            assertEquals(cryptoProps.getProperty(GmTestConfig.HASH_ALGORITHM), expectHash);
            assertEquals(expectHash, getField(testCrypto, "hashAlgorithm"));
            assertEquals(cryptoProps.getProperty(GmTestConfig.SECURITY_LEVEL), "384");

            // Should be exactly same instance as it has the same properties.
            assertEquals(testCrypto, GmCryptoSuiteFactory.getDefault().getCryptoSuite(propsIn));
    ...
    public void testSecurityLevel() throws InvalidArgumentException, IllegalAccessException, InstantiationException, ClassNotFoundException {
    	GmCryptoPrimitives testCrypto = new GmCryptoPrimitives();
    ...
    public void testGetTrustStore() throws IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        // been called
        try {
            GmCryptoPrimitives myCrypto = new GmCryptoPrimitives();
    ...
    public void testValidateCertificateByteArray() {
        assertTrue(crypto.validateCertificate(bytePemCert));
    }
    ...
    public void testBytesToCertificateInvalidBytes() throws CryptoException {
        crypto.bytesToCertificate(invalidPemCert);
    }
    ...
    public void testBytesToPrivateKeyInvalidBytes() throws CryptoException {
        crypto.bytesToPrivateKey(invalidPemCert);
    }
    ...
    public void testValidateCertificate() {
        try {
            BufferedInputStream pem = new BufferedInputStream(new ByteArrayInputStream(bytePemCert));
            X509Certificate cert = (X509Certificate) cf.engineGenerateCertificate(pem);
    ...
    public void testVerifyNullInput() {
        try {
        	byte[] b = null;
            assertFalse(crypto.verify(b, config.getSimpleValue("org.bcia.javachain.sdk.crypto.default_signature_algorithm"), null, null));
        } catch (CryptoException e) {
    ...
    public void testVerifyBadCert() throws CryptoException {
        ...
        crypto.verify(badCert, config.getSimpleValue("org.bcia.javachain.sdk.crypto.default_signature_algorithm"), sig, plainText);
    } // testVerifyBadCert

    @Test (expected = CryptoException.class)
    public void testVerifyBadSig() throws CryptoException {
        byte[] badSig = new byte[] {(byte) 0x00};
        crypto.verify(bytePemCert, config.getSimpleValue("org.bcia.javachain.sdk.crypto.default_signature_algorithm"), badSig, plainText);
    } // testVerifyBadSign
    ...
    public void testVerifyBadPlaintext() {
        ...
        try {
            assertFalse(crypto.verify(bytePemCert, config.getSimpleValue("org.bcia.javachain.sdk.crypto.default_signature_algorithm"), sig, badPlainText));
        } catch (CryptoException e) {
    ...
    public void testVerify() {
        try {
            assertTrue(crypto.verify(bytePemCert, config.getSimpleValue("org.bcia.javachain.sdk.crypto.default_signature_algorithm"), sig, plainText));
        } catch (CryptoException e) {
...
//            bis.close();
            ...
            assertTrue(((GmCryptoPrimitives)crypto).verify(crypto.getTrustStore().getCertificate("ca"), config.getSimpleValue("org.bcia.javachain.sdk.crypto.default_signature_algorithm"), signature, plainText));
    ...
    public void testKeyGenBadCrypto() throws CryptoException, IllegalAccessException, InstantiationException, ClassNotFoundException {
        ...
        GmCryptoPrimitives tmpCrypto = new GmCryptoPrimitives();
        tmpCrypto.keyGen();
    }
    ...
    @Test
    public void testHashSM3() throws Exception {

        byte[] input = "TheQuickBrownFox".getBytes(UTF_8);
        String expectedHash = "a52bbbf6ed1f3f9b63e5ba7c672aa326fdbfff913cbe12cebbda7130731ffeb9";

        crypto.setHashAlgorithm("SM3");
